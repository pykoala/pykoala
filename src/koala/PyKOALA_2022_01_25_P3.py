...
version="Version 1.07 - 25 January 2022 - Really last one before breaking code"
# This is Python 3.7
...
class Interpolated_cube(object):                       # TASK_Interpolated_cube
    ...
    RSS, pixel_size_arcsec, kernel_size_arcsec,    path="",
...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
#     def trace_peak_old(self, box_x=[0,-1],box_y=[0,-1], edgelow=-1, edgehigh=-1, odd_number = 21, adr_index_fit=2,
#                    plot=False, check_ADR=False, verbose = True):     
        
#         if self.grating == "580V":
#             if edgelow == -1: edgelow = 150
#             if edgehigh == -1: edgehigh = 50
#         else:
#             if edgelow == -1: edgelow = 40
#             if edgehigh == -1: edgehigh = 40

#         if verbose: print("\n> Tracing intensity peak over the wavelength range [{:.2f} , {:.2f}]...".format(self.wavelength[edgelow],self.wavelength[-1-edgehigh]))
...
#             x = np.arange(x1-x0)
#             y = np.arange(y1-y0)
#             if verbose: print("  As requested, using box at [{}:{} , {}:{}]".format(x0,x1,y0,y1))    
#             tmp=copy.deepcopy(self.data[:,y0:y1,x0:x1])
#         else:
#             tmp=copy.deepcopy(self.data)
#             x = np.arange(self.n_cols)
#             y = np.arange(self.n_rows)
#         if check_ADR and verbose: print("  Also checking ADR correction ...")   
    
#         # Matt's procedure   # FORO
...
#         yw = y[np.newaxis, :, np.newaxis] * weight
#         w = np.nansum(weight, axis=(1, 2))
#         self.x_peaks = np.nansum(xw, axis=(1, 2)) / w
#         self.y_peaks = np.nansum(yw, axis=(1, 2)) / w
#         self.x_peak_median = np.nanmedian(self.x_peaks)
#         self.y_peak_median = np.nanmedian(self.y_peaks)
#         self.x_peak_median_index = np.nanargmin(np.abs(self.x_peaks-self.x_peak_median)) 
#         self.y_peak_median_index = np.nanargmin(np.abs(self.y_peaks-self.y_peak_median))        

#         wl = self.wavelength
#         x = (self.x_peaks -self.x_peaks[self.x_peak_median_index] ) #*self.pixel_size_arcsec
#         y = (self.y_peaks -self.y_peaks[self.y_peak_median_index] ) #*self.pixel_size_arcsec
        

#         if np.nanmedian(box_x+box_y) != -0.5:
#             # Move peaks from position in box to position in cube
#             self.x_peaks = self.x_peaks + box_x[0]
#             self.y_peaks = self.y_peaks + box_y[0]
#             self.x_peak_median = self.x_peak_median + box_x[0]
#             self.y_peak_median = self.y_peak_median + box_y[0]
...
#         if verbose: print('\n  Using medfilt window = ',odd_number, "and fitting a polynomium of order", adr_index_fit)
#         # fit, trimming edges
...
#                 p=np.polyfit(wlm[fit_index], wx[fit_index], adr_index_fit)    # adr_index_fit was 2
#                 pp=np.poly1d(p)
#                 fx=pp(wl)
#                 fxm=pp(wlm)
#                 resid=wx-fxm
...
#                 if niter >= maxit and verbose: print("  x: Max iterations, {:2}, reached!".format(niter))
#                 if fit_len_init == fit_len and verbose: print("  x: All interval fitted in iteration {:2} ! ".format(niter))
#                 stop=2     
#             niter=niter+1
 

    
# #        if check_ADR == False: self.ADR_x_fit=np.polyfit(wl, fx/self.pixel_size_arcsec, adr_index_fit) 
#         if check_ADR == False: self.ADR_x_fit=np.polyfit(wl, fx, adr_index_fit) 
...
#                 fit_len=len(fit_index)
#             try:
#                 p=np.polyfit(wlm[fit_index], wy[fit_index], adr_index_fit)   # adr_index_fit was 2
#                 pp=np.poly1d(p)
#                 fy=pp(wl)
#                 fym=pp(wlm)
#                 resid=wy-fym
...
#                 if verbose: print('  Skipping iteration ',niter)    
#             if (niter >= maxit) or (fit_len_init == fit_len):
#                 if niter >= maxit and verbose: print("  y: Max iterations, {:2}, reached!")
#                 if fit_len_init == fit_len and verbose: print("  y: All interval fitted in iteration {:2} ! ".format(niter))
#                 stop=2
#             niter=niter+1
        
#         #if check_ADR == False: self.ADR_y_fit=np.polyfit(wl, fy/self.pixel_size_arcsec, adr_index_fit)  
#         if check_ADR == False: self.ADR_y_fit=np.polyfit(wl, fy, adr_index_fit)  
            
#         ADR_x=fx  
#         ADR_y=fy  
#         ADR_x_max = np.nanmax(ADR_x[edgelow:-1-edgehigh]) -np.nanmin(ADR_x[edgelow:-1-edgehigh])        
#         ADR_y_max = np.nanmax(ADR_y[edgelow:-1-edgehigh]) -np.nanmin(ADR_y[edgelow:-1-edgehigh]) 
#         ADR_total = np.sqrt(ADR_x_max**2 + ADR_y_max**2)        
        
#         if np.nanmedian(box_x+box_y) != -0.5 : self.plot_box_map(x0,x1,y0,y1)
            
#         if plot:    # This should be plot_plot
#             plt.figure(figsize=(10, 5))
#             plt.axhline(y=0, color='k', linestyle='--', alpha=0.5)

#             plt.plot(wl, fx, '-g', linewidth=3.5) 
#             plt.plot(wl, fy, '-g', linewidth=3.5)             
#             #plt.plot(wlm, fxm, "-b", linewidth=3.5, alpha=0.5)
#             plt.plot(wl, x, 'k.', label="RA", alpha=0.2)
#             plt.plot(wl, y, 'r.', label="Dec",alpha=0.2)
#             plt.plot(wl, signal.medfilt(x, odd_number), 'k-')
#             plt.plot(wl, signal.medfilt(y, odd_number), 'r-')
#             plt.axvline(x=valid_wl[0], color='k', linestyle='--', alpha=0.3)
#             plt.axvline(x=valid_wl[-1], color='k', linestyle='--', alpha=0.3)
            
#             hi = np.max([np.nanpercentile(x, 98), np.nanpercentile(y, 98)]) 
#             lo = np.min([np.nanpercentile(x, 2), np.nanpercentile(y, 2)])
#             plt.ylim(lo, hi)
#             plt.ylabel("$\Delta$ offset [arcsec]")
#             plt.xlabel("Wavelength [$\mathrm{\AA}$]")
#             plt.title(self.description)
#             plt.minorticks_on()  
#             plt.legend(frameon=True, loc=1, ncol=2)
#             plt.show()
#             plt.close()
...
#             if check_ADR == False: 
#                 plot_plot(wl, [x-fx, y-fy], ylabel="$\Delta$ offset [arcsec]", ptitle="Centroid - Fit",
#                           ymin=-0.2, ymax=0.2, #percentile_min=0.2, percentile_max=99.2, 
#                           fig_size=10, color=["k", "r"], hlines=[0] )
                        
            
#         if verbose: print("\n> Peak coordinates tracing the selected wavelength range found in spaxel: ({:.2f}, {:.2f})"\
#             .format(self.x_peak_median, self.y_peak_median ))  
#         if verbose: print("  Effect of the ADR in range : {:.2f} in RA (black), {:.2f} in DEC (red),  TOTAL = +/- {:.2f} arcsec".format(ADR_x_max,ADR_y_max, ADR_total))     
#         # if check_ADR :        
#         #     if verbose: print("           --> Before it was : {:.2f} in RA,         {:.2f} in DEC,        TOTAL = +/- {:.2f} arcsec".format(self.ADR_x_max,self.ADR_y_max, self.ADR_total))     
#         #     self.ADR_x = ADR_x      # ALL OF THESE HAD _NEW at the left hand size
#         #     self.ADR_y = ADR_y 
#         #     self.ADR_x = ADR_x_max      
#         #     self.ADR_y = ADR_y_max
#         #     self.ADR_total = ADR_total 
#         # else:
#         self.ADR_x=ADR_x  
#         self.ADR_y=ADR_y
...
#         self.ADR_y_max = ADR_y_max
#         self.ADR_total = ADR_total
#         if self.ADR_total > self.pixel_size_arcsec*0.5  :
#             if verbose: print("  The combined ADR is larger than half pixel size! Applying this ADR correction is needed !!")
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
    def get_bad_spaxels(self, verbose = False, show_results = True, valid_wave_min = 0, valid_wave_max = 0):
        ...
        show_results : TYPE, optional
            DESCRIPTION. The default is True.
        valid_wave_min : TYPE, optional
            DESCRIPTION. The default is 0.
        valid_wave_max : TYPE, optional
            DESCRIPTION. The default is 0.
...
# def clean_cosmics_and_extreme_negatives(rss, fibre_list="", 
#                                         clean_cosmics = True, only_positive_cosmics= True,
#                                         disp_scale=1.0, disp_to_sqrt_scale = 1.6, ps_min = 3, width = 20,  
#                                         remove_negative_median_values = False,                                         
#                                         clean_extreme_negatives = True, percentile_min = 0.5,
#                                         show_correction_map = True,
#                                         show_fibres=[0,100,500,900], 
#                                         show_cosmics_identification = False,
#                                         plot=False, verbose=False):
#     """
#     The LOWER the disp_to_sqrt_scale is, the MORE residua eliminates, 
#     if it is too low (below 1.5) it might remove REAL features, 
#     if it is too high (more than 10) it will not remove some real cosmics
#     """
#     g = copy.deepcopy(rss.intensity_corrected)
#     w = rss.wavelength
#     n_wave = rss.n_wave
#     minimo=np.nanpercentile(g, percentile_min)

#     if fibre_list =="" : 
#         fibre_list=list(range(rss.n_spectra))
#         fibre_list_ALL = True
#     else:
#         if len(fibre_list) < 10: show_fibres=fibre_list
#         fibre_list_ALL = False
    
#     if  clean_cosmics :             
#         if fibre_list_ALL  : 
#             print("\n> Cleaning cosmics in all fibres ...")
#             rss.history.append("- Cosmics cleaned in all fibres using values:")
#         else:
#             print("\n> Cleaning cosmics in given fibres : ")
#             print("  ",fibre_list," ...")
#             rss.history.append("- Cosmics cleaned in selected fibres:")
#             rss.history.append(fibre_list)
        
        
#         rss.RSS_image(title=" - RSS image BEFORE cleaning cosmics")
        
#         print("  The limits are set using :")
#         print("   +- [ disp_scale * dispersion (fibre) + 100 * disp_to_sqrt_scale * dispersion (fibre) / sqrt (integrated flux fibre) ]")     
#         print("  and peak / suma > ps_min,  being peak = min(f[region]) - min (f_median[region])")
#         print("  and suma =  ( sum(f[region]) - sum(f_median[region])) / len(region)")
#         print("  The region is given by [ cosmic-width/3, cosmic+width/3]") 
       
#         print("\n  Values provided :  disp_scale =",disp_scale," , disp_to_sqrt_scale =",disp_to_sqrt_scale," ,  ps_min =", ps_min, " ,  width =",width,"\n")
#         rss.history.append("  disp_scale = "+np.str(disp_scale)+", disp_to_sqrt_scale = "+np.str(disp_to_sqrt_scale)+", ps_min = "+np.str(ps_min)+", width = "+np.str(width)+" A")
                    
        
#         # TO INCLUDE HERE: AVOID EMISSION LINES!!!            
        
        
#         say_status=[100,200,300,400,500,600,700,800,900]
#         n_cosmics_found=[]
#         n_negatives_found=[]
#         plot_here = False
#         for fibre in fibre_list:
#             if fibre in say_status or fibre in show_fibres : 
#                 print(" - Processing fibre ",fibre,"...")  
#                 plot_here = True
#             else:
#                 plot_here= False
#             f = rss.intensity_corrected[fibre]
#             fm=signal.medfilt(f,101) 
#             # This is the important part:
#             # Decide how to make the good cut of cosmics!!!
#             raiz= np.sqrt(np.nansum(f)) #, np.sqrt(np.nansum(fm))
#             dispersion = np.nanstd(f)
#             #cut = 0.25 now sqrt_scale
#             #corte = cut * raiz - 1. * dispersion
#             #corte = sqrt_scale * raiz - ( disp_scale * dispersion ) + 100. * dispersion/raiz                
#             corte = disp_scale * dispersion + disp_to_sqrt_scale * 100 * dispersion/raiz                
#             if fibre in show_fibres: 
#                 print("    dispersion              = ",np.round(dispersion,2)," ,   root (integrated flux) = ",np.round(raiz,2))
#                 print("    disp_scale * dispersion = ",np.round((disp_scale * dispersion),2)," ,        dispersion / root = ", np.round(dispersion/raiz,2),"         -> cut = +/-",np.round(corte,2),"\n")                
#             if only_positive_cosmics:
#                 bad_pixels_=np.where((f-fm) > corte)                   
#             else:
#                 bad_pixels_=np.where(np.abs(f-fm) > corte )
#             bad_pixels=bad_pixels_[0]
#             rangos=[]
#             if len(bad_pixels) > 0:
#                 este_rango=[bad_pixels[0]]
#                 for i in range(len(bad_pixels) -1):
#                     if bad_pixels[i]+1 == bad_pixels[i+1]:
#                         este_rango.append(bad_pixels[i+1])
#                     else:
#                         rangos.append(este_rango)
#                         este_rango=[bad_pixels[i+1]]
#                 rangos.append(este_rango)
#                 vlines=[]  # Here we list the center of the cosmics/negatives to be corrected
#                 n_cosmics = 0
#                 n_negatives = 0 
#                 for rango in rangos:
#                     if rango[0]-9 > 0 and rango[-1] < n_wave-9:  # Forget the edges
#                         region=list(range(np.int(rango[0]-width/3.),np.int(rango[-1]+width/3.)))
#                         suma=(np.nansum(f[region])-np.nansum(fm[region]))/len(region)
#                         if suma < 0:
#                             peak=(np.nanmin(f[region])-np.nanmin(fm[region]))
#                         else:
#                             peak=(np.nanmax(f[region])-np.nanmax(fm[region]))
#                         #if fibre in show_fibres:
#                         if peak/suma > ps_min: 
#                             vlines.append(w[np.int(np.nanmedian(rango))])
#                             if suma > 0 : 
#                                 n_cosmics = n_cosmics +1
#                             else:
#                                 n_negatives = n_negatives +1
#                             if verbose or fibre in show_fibres: 
#                                 print("    - cosmic ",n_cosmics," at wavelength = ",np.round(w[np.int(np.nanmedian(rango))],2)," A")
#                                 print("                 range : ",rango, "  central = ", np.int(np.nanmedian(rango)))
#                                 print("                 sum = ",np.round(suma,2), ",    peak = ",np.round(peak,2), ",                      -> peak/sum = ",np.round(peak/suma,2)) 
                            
#                 n_cosmics_found.append(n_cosmics)
#                 n_negatives_found.append(n_negatives)   
#             else:
#                 vlines=[]
...
#             s=copy.deepcopy(f)
#             for line in vlines:
#                 s = get_continuum_in_range(w,s,line-60, line-width/2, line+width/2, line+60,
#                                pmin=12,pmax=88, only_correct_negative_values = False,
#                                fit_degree=2, plot = False, verbose = True)                
                
#             #if plot or fibre in show_fibres:
#             if plot_here:    
#                 if show_cosmics_identification: 
#                     ptitle = "Cosmics identification in fibre "+np.str(fibre)
#                     plot_plot(w,f-fm, hlines=[-corte,corte],vlines=vlines, 
#                               ymin= -corte-60, ymax= corte+60,
#                               ptitle=ptitle, ylabel="Flux - Median Flux")
#                 ptitle = "Correcting cosmics in fibre "+np.str(fibre)
#                 plot_plot(w,[f,s] ,vlines=vlines, hlines=[minimo], ptitle=ptitle)
#             g[fibre] = s
        
#         total_cosmics = np.nansum(n_cosmics_found)
#         print("\n> Found ",total_cosmics," cosmics in total !")
#         rss.history.append("  Cleaned a total of "+np.str(total_cosmics)+" cosmics")
#         if only_positive_cosmics == False:
#             print("\n> Found ",np.nansum(n_negatives_found)," negatives in total !")
#             rss.history.append("  Cleaned a total of "+np.str(np.nansum(n_negatives_found))+" negative values")               
#     else:
#          print("\n> No cleaning of the cosmics requested.")
       
#     # Correcting extreme negatives       
#     if remove_negative_median_values:
#         #print("\n> Removing negative pixels in the sky spectra...") 
#         #rss.intensity_corrected = remove_negative_pixels(rss.intensity_corrected) #GAFAS
#         g = remove_negative_pixels(g) 
    
#                 # self.intensity_corrected = remove_negative_pixels(self.intensity_corrected) #GAFAS
#                 # for i in range(self.n_spectra):
#                 #     self.integrated_fibre[i] = np.nansum(self.intensity_corrected[i,region])   
    
#     if clean_extreme_negatives:
#         print("\n> Now correcting the extreme negatives, making any pixel below") 
#         print("  np.nanpercentile(intensity_corrected, ", percentile_min, ") = ",np.round(minimo,2))
#         print("  to have the median value of the fibre...")           
#         if fibre_list_ALL == False: print("  Doing this only in selected fibres...")
        
#         for fibre in fibre_list:
#             g[fibre] = [np.nanmedian(g[fibre]) if x < minimo else x for x in g[fibre]  ]
#         rss.history.append("- Extreme negatives (values below percentile "+np.str(np.round(percentile_min,3))+" = "+np.str(np.round(minimo,3))+" ) cleaned")                  
#     else:
#         print("\n> No correction of extreme negatives (values below percentile = "+np.str(np.round(percentile_min,3))+" = "+np.str(np.round(minimo,3))+" ) requested.")

#     if show_correction_map:
#         correction_map = g / rss.intensity_corrected #/ g
#         rss.RSS_image(image=correction_map, cmap="binary_r", title=" - Correction map")
