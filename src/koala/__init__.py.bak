...
class RSS(object):
    ...
    def correct_high_cosmics_and_defects(
            ...
            for corte in range(cuts):
                if corte == 0:
                    ...
                    next_wave = np.nanmedian(
                        (wlm[np.int(corte * step)] + wlm[np.int((corte + 1) * step)])
                        / 2
                    )
                if next_wave < wave_max:
                    running_wave.append(next_wave)
                    region = np.where(
                        (wlm > running_wave[corte] - step / 2)
                        & (wlm < running_wave[corte] + step / 2)
                    )
        ...
        if fibre_p > 0:
            ...
            plt.plot(
                wlm,
                espectro_old / espectro_fit_median,
                "r",
                label="Uncorrected",
                alpha=0.5,
            )
            plt.plot(
                wlm,
                espectro_new / espectro_fit_median,
                "b",
                label="Corrected",
                alpha=0.5,
            )
        ...
        if plot:
            ...
            for fibre in range(self.n_spectra):
                ...
                if skip == 0:
                    median_value = np.nanmedian(
                        self.integrated_fibre[
                            fibre - np.int(step_f / 2) : fibre + np.int(step_f / 2)
                        ]
                    )
                median_running.append(median_value)

                if self.integrated_fibre[fibre] / median_running[fibre] > max_value:
                    print "  Fibre ", fibre, " has a integrated/median ratio of ", self.integrated_fibre[
                        fibre
                    ] / median_running[
                        fibre
        ...
        if plot_suspicious_fibres == True and len(suspicious_fibres) > 0:
            ...
            for fibre in suspicious_fibres:
                ...
                scale = np.nanmax(self.intensity_corrected[fibre]) - np.nanmin(
                    self.intensity_corrected[fibre]
                )
                plt.ylim(
                    np.nanmin(self.intensity_corrected[fibre]) - scale / 15,
                    np.nanmax(self.intensity_corrected[fibre]) + scale / 15,
                )
                text = (
    ...
    def fit_and_substract_sky_spectrum(
        ...
        w=1000,
        spectra=1000,
        rebin=False,  # If rebin == True, it fits all wavelengths to be at the same wavelengths that SKY spectrum...
        brightest_line="Ha",
        ...
        for fibre in range(f_i, f_f):  #    (self.n_spectra):
            ...
            for i in range(number_sl):
                ...
                ratio_object_sky_sl_gaussian.append(
                    object_sl_gaussian_flux[i] / sl_gaussian_flux[i]
                )
    ...
    def line_flux(
        ...
        left, var_left = self.flux_between(left_min, left_max, list_spectra) / (
            left_max - left_min
        )
        right, var_right = self.flux_between(right_min, right_max, list_spectra) / (
            left_max - left_min
        )
        wavelength_left = (left_min + left_max) / 2
        wavelength_line = (line_min + line_max) / 2
        wavelength_right = (right_min + right_max) / 2
        continuum = left + (right - left) * (wavelength_line - wavelength_left) / (
    ...
    def __init__(
        ...
        if flat != "":
            ...
            self.intensity_corrected = (
                self.intensity_corrected / flat.intensity_corrected
            )
        ...
        if sky_method == "1D" or sky_method == "2D":
            remove_5578 = False  # If not it will not work when applying scale for sky substraction...
                ...
                if scale_sky_rss != 0:
                    ...
                    if sky_line_2 == 0:
                        ...
                        scale_sky_rss = np.nanmedian(
                            (scale_per_fibre + scale_per_fibre_2) / 2
                        )
                        # Make linear fit
                        ...
                        b = (scale_sky_rss_1 - scale_sky_rss_2) / (
                            sky_line - sky_line_2
                        )
...
class Interpolated_cube(object):  # TASK_Interpolated_cube
    ...
    def __init__(
        ...
        self.kernel_size_pixels = (
            kernel_size_arcsec / pixel_size_arcsec
    ...
    def add_spectrum(self, intensity, offset_rows, offset_cols, warnings=False):
        ...
        x = (
            np.linspace(x_min - kernel_centre_x, x_max - kernel_centre_x, n_points_x)
            / self.kernel_size_pixels
        )
        x[0] = -1.0
        x[-1] = 1.0
        ...
        y = (
            np.linspace(y_min - kernel_centre_y, y_max - kernel_centre_y, n_points_y)
            / self.kernel_size_pixels
        )
        y[0] = -1.0
        y[-1] = 1.0
    ...
    def growth_curve_between(self, min_wave=0, max_wave=0, plot=False, verbose=False):
        ...
        if plot:
            ...
            print "      Flux guess =", F_guess, np.nansum(
                intensity
            ), " ratio = ", np.nansum(intensity) / F_guess
    ...
    def do_response_curve(
        ...
        _response_curve_ = (
            measured_counts / flux_cal / exp_time
...
def fit_Moffat(
    ...
    if plot:
        ...
        plt.plot(
            r_norm,
            cumulaive_Moffat(np.array(r2_growth_curve), fit[0], fit[1], fit[2])
            / fit[0],
            ":",
        )
...
def save_rss_fits(
    ...
    header2_hdu.header["CENRA"] = rss.RA_centre_deg / (
        180 / np.pi
    )  # Must be in radians
...
def spectrum_to_fits_file(
    ...
    if CRVAL1_CDELT1_CRPIX1[0] == 0:
        print "  CRVAL1_CDELT1_CRPIX1 values not given, using ", wavelength[0], "1", (
    ...
    for corte in range(cuts + 1):
        ...
        if next_wave < wave_max:
            ...
            if (
                ...
                # print running_wave
                region = np.where(
                    (wlm > running_wave[corte_index] - step / 2)
                    & (wlm < running_wave[corte_index] + step / 2)
                )
...
def offset_positions(
    ...
    if dec1d < 0:
        ...
        dec1 = dec1d + dec1m / 60.0 + dec1s / 3600
    if dec2d < 0:
