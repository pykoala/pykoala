...
def spectrum_to_text_file(wavelength, flux, filename):
    """
    ...
    np.savetxt(filename, np.array([wavelength, flux]).T,
               header="Wavelength, Flux")
    vprint(f'Spectrum saved in text file : {filename}')

def create_1d_spectra_wcs(wavelength=None, unit="Angstrom", cdelt=None,
                          crpix=None, crval=None, naxis=None):
    """
    Create a WCS for a 1D spectra. #TODO: Add documentation
    """
    if cdelt is None and wavelength is not None:
        cdelt = wavelength[1] - wavelength[0]
    if crpix is None and wavelength is not None:
        crpix = 0
        crval = wavelength[0]
    if crval is None:
        crval = wavelength[crpix]
    if naxis is None and wavelength is not None:
        naxis = len(wavelength)

    wcs_dict = {"CTYPE1": "WAVE    ", "CUNIT1": unit, "CDELT1": cdelt,            
                "CRPIX1": crpix, "CRVAL1": crval, "NAXIS1": naxis}
    return wcs.WCS(wcs_dict)

def spectrum_to_fits_file(wavelength, flux, wcs=None, filename="spectrum.fits",
                          name="spectrum",  exptime=1):
    """
    ...
    hdu = fits.PrimaryHDU()
    if wcs is None:
        wcs = create_1d_spectra_wcs(wavelength)
    header = wcs.to_header()
    hdu.data = (flux)
    hdu.header = header
    hdu.header['ORIGIN'] = 'PyKOALA'

    # Extra info
    hdu.header['OBJECT'] = name, "Object alias"
    hdu.header["TOTALEXP"] = exptime, "Total exposure time"
    hdu.header['HISTORY'] = 'Spectrum derived using the PyKOALA pipeline'
    hdu.header['HISTORY'] = datetime.datetime.now().strftime(
        "%Y-%m-%dT%H:%M:%S"), "Creation date"
    hdu.writeto(filename, overwrite=True) 
    vprint(f"1D Spectrum saved in fits file: {filename}")

def calculate_z(l_obs=None, l_ref=None, ref_line = None, v_rad=None):
    """
