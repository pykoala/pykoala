...
class ParabolicKernel(InterpolationKernel):
    ...
    def kernel_1D(self, x_edges):
        ...
        """
        assert (x_edges[1:] > x_edges[:-1]).all(), "Input x_edges values must be increasing"
    ...
    def kernel_2D(self, x_edges, y_edges):
        ...
        for i, pos in enumerate(zip(pix_edge_x.flatten(), pix_edge_y.flatten())):
            _, area_fraction = ancillary.pixel_in_circle(
                pos, pixel_size=1, circle_pos=(0, 0),
...
def interpolate_rss(rss, wcs, kernel,
    ...
    cube_wavelength = wcs.spectral.array_index_to_world(
        np.arange(wcs.array_shape[0])).to(rss.wavelength.unit)
    ...
    if adr_ra_arcsec is not None:
        ...
        if cube_wavelength.size != rss.wavelength.size or np.allclose(
        cube_wavelength, rss.wavelength):
            adr_ra_pixel = np.interp(
                cube_wavelength, rss.wavelength, adr_ra_pixel)
    else:
        adr_ra_pixel = None
...
def build_cube(rss_set, wcs=None, wcs_params=None, kernel=GaussianKernel,
    ...
    for ith, rss in enumerate(rss_set):
        ...
        datacube_i, datacube_var_i, datacube_weight_i, rss_plots = interpolate_rss(
            ...
            adr_ra_arcsec=adr_set[ith][0], adr_dec_arcsec=adr_set[ith][1],
...
def build_wcs_from_rss(rss_list : list, spatial_pix_size : u.Quantity,
    ...
    **kwargs : dict
    ...
    if join_type == "outer":
        ...
        min_wl, max_wl = (np.nanmin(rss_spectral_range[:, 0]),
        ...
        min_wl, max_wl = (np.nanmax(rss_spectral_range[:, 0]),
    ...
    vprint("Combined footprint Fov: {:.2f}, {:.2f} (arcmin)".format(ra_width.to("arcmin"),
                                                                dec_width.to("arcmin")))
    vprint("Combined footprint wavelength range: {:.1f}, {:.1f} (AA)".format(
        min_wl.to("AA"), max_wl.to("AA")))

    datacube_shape = (int((wl_range / spectra_pix_size).decompose().value),
                      int((ra_width / spatial_pix_size).decompose().value),
                      int((dec_width / spatial_pix_size).decompose().value))
    reference_position = (min_wl, ra_cen, dec_cen)
    vprint(f"WCS array shape: {datacube_shape} [wave, ra, dec]")
    return build_wcs(datacube_shape=datacube_shape, reference_position=reference_position,
                     spatial_pix_size=spatial_pix_size, spectra_pix_size=spectra_pix_size,
                     **kwargs)
...
def make_dummy_cube_from_rss(rss, spa_pix_arcsec=0.5, kernel_pix_arcsec=1.0):
    ...
    wcs = build_wcs_from_rss(
        rss, spatial_pix_size=spa_pix_arcsec,
